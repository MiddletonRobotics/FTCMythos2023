...
@Autonomous(name="BP2-VISION-ParkLeft")

public class BlueVisionPixelPlacement extends OpMode {
    ...
    public void init() {
        ...
        Rect leftZone = centerRect(550, 290, 150, 260);
        Rect rightZone = centerRect(300, 220, 270, 150);
        ...
        if (detector.isConfident()) {
            if (h == "RIGHT") {
                ...
                if (RunTime.seconds() < 2.65) {
                    ...
                    AsterionMotors.mecanumDrive(-Constants.AutoSpeed, 0, 0);
                } else if (RunTime.seconds() > 5.82 && RunTime.seconds() < 6) {
                    AsterionMotors.mecanumDrive(0, 0, 0);
                } else if (RunTime.seconds() > 6 && RunTime.seconds() < 6.4) {
                    AsterionMotors.mecanumDrive(Constants.AutoSpeed, 0, 0);
                } else if (RunTime.seconds() > 6.4 && RunTime.seconds() < 6.6) {
                    AsterionMotors.mecanumDrive(0, 0, 0);
                } else if (RunTime.seconds() > 6.6 && RunTime.seconds() < 7.5) {
                    AsterionMotors.mecanumDrive(0, 0, -0.3);
                } else if (RunTime.seconds() > 7.5 && RunTime.seconds() < 7.7) {
                    AsterionMotors.mecanumDrive(0, 0, 0.0);
                } else if (RunTime.seconds() > 7.7 && RunTime.seconds() < 9.3) {
                    AsterionMotors.mecanumDrive(Constants.AutoSpeed, 0, 0.0);
                } else if (RunTime.seconds() > 9.3 && RunTime.seconds() < 9.5) {
                    AsterionMotors.mecanumDrive(0, 0, 0.0);
                }
            }
        }
