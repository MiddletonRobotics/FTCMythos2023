...
@Autonomous(name="BP1-Middle")
@Disabled

public class BPMiddle extends OpMode {
    ...
    public void loop() {
        if (RunTime.seconds() < 3.15) {
            ...
            AngleMotor.setPower(-0.3);
            AsterionMotors.mecanumDrive(0, 0, 0);
        } else if (RunTime.seconds() > 5.2 && RunTime.seconds() < 6.4) {
            AngleMotor.setPower(-0.3);
            AsterionMotors.mecanumDrive(0, 0, 0.3);
        } else if (RunTime.seconds() > 6.4 && RunTime.seconds() < 8.9) {
            AngleMotor.setPower(-0.3);
            AsterionMotors.mecanumDrive(-Constants.AutoSpeed, 0, 0);
        } else if (RunTime.seconds() > 8.9 && RunTime.seconds() < 9.1) {
            AngleMotor.setPower(-0.3);
            AsterionMotors.mecanumDrive(0, 0, 0);
        } else if (RunTime.seconds() > 9.1 && RunTime.seconds() < 11.2) {
            AngleMotor.setPower(-0.3);
            LiftMotor.setPower(-0.6);
        } else if (RunTime.seconds() > 11.2 && RunTime.seconds() < 11.2) {
